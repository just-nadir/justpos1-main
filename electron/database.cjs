...
function initDB() {
  ...
  // --- MIGRATSIYA (Jadvalga yangi ustun qo'shish) ---
  try {
    const columns = db.prepare(`PRAGMA table_info(kitchens)`).all();
    const hasType = columns.some(c => c.name === 'printer_type');
    if (!hasType) {
        db.prepare(`ALTER TABLE kitchens ADD COLUMN printer_type TEXT DEFAULT 'lan'`).run();
        console.log("MIGRATION: 'printer_type' ustuni 'kitchens' jadvaliga qo'shildi.");
    }
  } catch (err) {
  ...
  if (stmtK.get().count === 0) {
     // Default kitchens (endi type bilan)
     ...
     insertK.run('Mangal', '', 'driver'); // Misol uchun
  }
